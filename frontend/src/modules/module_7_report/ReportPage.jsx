    ...
    <div className="space-y-5 max-w-lg mx-auto">
      ...
      {reports.length === 0 && !loading ? (
        <div className="bg-white rounded-2xl shadow-sm border border-[#F0EDE8] px-8 py-12 text-center space-y-4">
          ...
          <p className="text-xs text-gray-400">Complétez les étapes du parcours pour générer votre premier carnet de style.</p>
          {/* Guide pas-à-pas pour orienter l'utilisateur */}
          <div className="text-left space-y-2 mt-2">
            {[
              { step: '1', label: 'Prenez vos mesures', path: '/modules/2' },
              { step: '2', label: 'Choisissez tissu & patron', path: '/modules/3' },
              { step: '3', label: 'Calculez les marges d\'aisance', path: '/modules/5' },
              { step: '4', label: 'Vérifiez la compatibilité', path: '/modules/6' },
            ].map(({ step, label, path }) => (
              <button
                key={step}
